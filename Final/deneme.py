...
def get_robot_pose(robot_handle):
    ...
    # initial_yaw_offset_degrees = 89.308
    # initial_yaw_offset_radians = np.deg2rad(initial_yaw_offset_degrees)
    # # Çıkarma yerine toplamayı deneyin
    # corrected_yaw = robot_orientation[2] + initial_yaw_offset_radians
    corrected_yaw = robot_orientation[2]
    return np.array([robot_position[0], robot_position[1], corrected_yaw])
def navigate_to_marker(target_id, robot_handle, sensor_handle):
    angular_speed_factor = 1.5
    angle_threshold_for_move = 0.05
    linear_speed = 0.4  # Artırılmış lineer hız
    angular_speed = 0.05
    ...
    while True:
        ...
        print(f"Hedef: {target_id}, Ham Robot Açısı: {sim.getObjectOrientation(robot_handle, -1)[2]:.2f}, Düzeltilmiş Robot Açısı: {robot_pose[2]:.2f}, Hedef Açı (Global): {angle_to_target_global:.2f}, Açı Farkı: {angle_difference:.2f}, Mesafe: {distance_to_target:.2f}")
