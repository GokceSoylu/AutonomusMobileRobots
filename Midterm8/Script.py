...
def execute():
    global e_old, e_integral, obstacle_detected_time, lap_count, lap_start_time, lap_times
    state = "FOLLOWING"
    ...
    while sim.getSimulationState() != sim.simulation_stopped:
        ...
        if pos[0] > 1 and lap_count == 0 and current_time - sim_start_time > 5:  # İlk tur tamamlandı
            lap_count = 1
            lap_time = current_time - sim_start_time
            lap_times.append(lap_time)
            print(f"1. Tur Tamamlama Süresi: {lap_time:.2f} saniye")
            lap_start_time = current_time
        elif pos[0] > 1 and lap_count == 1 and current_time - lap_start_time > 5:  # İkinci tur tamamlandı
            lap_count = 2
...
def start_simulation():
    ...
    execute()
    sim.stopSimulation()
    sim.stopSimulation() # Simülasyonu durdur
