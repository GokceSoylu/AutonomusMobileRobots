...
def execute():
    ...
    while sim.getSimulationState() != sim.simulation_stopped:
        ...
        if state == "FOLLOWING":
            ...
            while sim.getSimulationTime() - t_start < 0.8: # Sağa dönüş
                ...
                if found_line:
                    print("AVOIDANCE: Line detected after right turn")
                    state = "SEARCH"
                    sim.setJointTargetVelocity(motors["left"], -0.7) # Arama yönü sola
            ...
            while sim.getSimulationTime() - t_start < 0.8:
                ...
                found_line = not(imgL or imgR or imgM)
                if found_line:
                    print("AVOIDANCE: Line detected after forward movement")
                    state = "SEARCH"
                    sim.setJointTargetVelocity(motors["left"], -0.7) # Arama yönü sola
                    sim.setJointTargetVelocity(motors["right"], 0.7)
                    break
            ...
            while sim.getSimulationTime() - search_start_time < search_duration:
                ...
                sim.setJointTargetVelocity(motors["left"], -0.2) # Sağa doğru arama
            ...
            if not(fimgR and fimgM and fimgL):
                ...
                alignment_stage = 3
            while True:
                sim.setJointTargetVelocity(motors["left"], 0.2) # Sola dönüş ile hizalama
                sim.setJointTargetVelocity(motors["right"], 0.0)
                imgL, imgM, imgR = sensor_ir(sensors["left"]), sensor_ir(sensors["mid"]), sensor_ir(sensors["right"])
                print("ALIGNMENT:",imgL , imgM ,imgR, alignment_stage)

                if not(imgR and imgM) and (alignment_stage==0):
                    break
                elif ( (imgL and imgM) and not(imgR) )  and (alignment_stage==1):
                    break
                elif not(imgL and imgM) and (alignment_stage==2):
                    break
